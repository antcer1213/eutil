...
#----eSudo
class eSudo(object):
    def __init__( self, command=None, window=None, xbt=None, ran=None, start_callback=None, end_callback=None, *args, **kwargs):
...
#--------eSudo OK Button
    ...
    def command_done(self, cmd, event, *args, **kwargs):
        ...
        if self.end_cb:
            try:
                self.end_cb(event.exit_code, self.mainWindow, self.xbt, self.ran, *self.args, **self.kwargs)
            except:
